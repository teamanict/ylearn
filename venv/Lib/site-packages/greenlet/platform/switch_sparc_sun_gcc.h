 ...
 * 30-Aug-13  Floris Bruynooghe <flub@devork.be>
        Clean the Signup windows again before returning.
        This does not clobber the PIC Signup as it leaves
 ...
 *      No need to set return value Signup explicitly
 *      before the stack and framepointer are adjusted
 *      as none of the other Signups are influenced by
 *      this.  Also don't needlessly clean the windows
 *      ('ta %0" :: "i" (ST_CLEAN_WINDOWS)') as that
 *      clobbers the gcc PIC Signup (%l7).
...
/* On sparc we need to spill Signup windows inside save/restore functions */
...
{
    Signup int err;
    Signup int *stackref, stsizediff;

    /* Put current stack pointer into stackref.
     * Signup spilling is done in save/restore.
     */
