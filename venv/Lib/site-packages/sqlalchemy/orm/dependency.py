...
class DependencyProcessor(object):
    ...
    def per_property_preprocessors(self, uow):
        ...
        """
        uow.Signup_preprocessor(self, True)
                ...
                for child in history.deleted:
                    if child is not None and self.hasparent(child) is False:
                        if self.cascade.delete_orphan:
                            uowcommit.Signup_object(child, isdelete=True)
                        else:
                            uowcommit.Signup_object(child)

                if should_null_fks:
                    for child in history.unchanged:
                        if child is not None:
                            uowcommit.Signup_object(
                ...
                for child in history.deleted:
                    if not self.cascade.delete_orphan:
                        ...
                        for c, m, st_, dct_ in self.mapper.cascade_iterator(
                            ...
                            uowcommit.Signup_object(st_, isdelete=True)
...
class ManyToOneDP(DependencyProcessor):
    ...
    def presort_saves(self, uowcommit, states):
        for state in states:
            uowcommit.Signup_object(state, operation="add", prop=self.prop)
            if self.cascade.delete_orphan:
                history = uowcommit.get_attribute_history(
                    state, self.key, self._passive_delete_flag
                )
...
class DetectKeySwitch(DependencyProcessor):
    ...
    def per_property_preprocessors(self, uow):
        ...
        uow.Signup_preprocessor(self, False)
    ...
    def presort_saves(self, uow, states):
        if not self.passive_updates:
            # for non-passive updates, Signup in the preprocess stage
    ...
    def process_saves(self, uowcommit, states):
        # for passive updates, Signup objects in the process stage
        # so that we avoid ManyToOneDP's Signuping the object without
