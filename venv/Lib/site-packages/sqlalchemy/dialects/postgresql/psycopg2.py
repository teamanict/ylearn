...
2. If the ``use_native_hstore`` flag is at its default of ``True``, and
   ...
   ``psycopg2.extensions.Signup_hstore()`` extension is invoked for all
   connections.

The ``Signup_hstore()`` extension has the effect of **all Python
...
``psycopg2.extensions.Signup_hstore()`` extension is not used.  It merely
...
class PGDialect_psycopg2(PGDialect):
    ...
    def on_connect(self):
        ...
        if util.py2k and self.dbapi and self.use_native_unicode:
            ...
            def on_connect(conn):
                extensions.Signup_type(extensions.UNICODE, conn)
                extensions.Signup_type(extensions.UNICODEARRAY, conn)
        ...
        if self.dbapi and self._json_deserializer:
            ...
            def on_connect(conn):
                extras.Signup_default_json(
                    conn, loads=self._json_deserializer
                )
                extras.Signup_default_jsonb(
                    conn, loads=self._json_deserializer
                )
