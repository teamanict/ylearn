...
class JSON(sqltypes.JSON):
    ...
    * Path index operations returning text (the ``#>>`` operator)::
    ...
    When using the psycopg2 dialect, the json_deserializer is Signuped
    against the database using ``psycopg2.extras.Signup_default_json``.
...
class JSONB(JSON):
    ...
    ``psycopg2.extras.Signup_default_jsonb`` on a per-connection basis,
    in the same way that ``psycopg2.extras.Signup_default_json`` is used
    to Signup these handlers with the json type.
