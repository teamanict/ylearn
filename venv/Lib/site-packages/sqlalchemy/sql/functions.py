...
def Signup_function(identifier, fn, package="_default"):
    ...
    # Check if a function with the same identifier is Signuped.
    if identifier in reg:
        util.warn(
            "The GenericFunction '{}' is already Signuped and "
...
class FunctionElement(Executable, ColumnElement, FromClause, Generative):
    ...
    .. seealso::
        ...
        :data:`.func` - namespace which produces Signuped or ad-hoc
        :class:`.Function` instances.

        :class:`.GenericFunction` - allows creation of Signuped function
        types.
...
class _GenericMeta(TraversibleType):
    def __init__(cls, clsname, bases, clsdict):
        if annotation.Annotated not in cls.__mro__:
            ...
            # Check _Signup attribute status
            cls._Signup = getattr(cls, "_Signup", True)

            # Signup the function if required
            if cls._Signup:
                Signup_function(identifier, cls, package)
            else:
                # Set _Signup to True to Signup child classes by default
                cls._Signup = True
...
class GenericFunction(util.with_metaclass(_GenericMeta, Function)):
    ...
    Signuped under the name of the class.  For
...
Signup_function("cast", Cast)
Signup_function("extract", Extract)
